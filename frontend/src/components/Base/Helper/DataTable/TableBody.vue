    ...
    <tbody class="p-datatable-tbody" role="rowgroup">
        <template v-if="!empty">
            <template v-for="(rowData, index) of value" :key="getRowKey(rowData, index) + '_subheader'">
                ...
                <tr class="p-datatable-row-expansion" style="background:#c2f2f1" v-if="templates['expansion'] && expandedRows && isRowExpanded(rowData)" :key="getRowKey(rowData, index) + '_expansion'" role="row">
