<template>
    <div ref="container" :class="containerClass" @click="onClick($event)">
        ...
        <span v-if="!editable" :class="labelClass">
            <slot name="value" :value="modelValue" :placeholder="placeholder">{{modelValue?modelValue:label}}</slot>
        </span>
                ...
                <div :ref="overlayRef" class="p-dropdown-panel p-component" v-if="overlayVisible" @click="onOverlayClick">
                    ...
                    <div :ref="itemsWrapperRef" class="p-dropdown-items-wrapper" :style="{'max-height': scrollHeight}">
                        <ul class="p-dropdown-items" role="listbox">
                            ...
                            <li v-if="filterValue && (!visibleOptions || (visibleOptions && visibleOptions.length === 0)) && !loading" class="p-dropdown-empty-message">
                                <slot name="emptyfilter">{{emptyFilterMessageText}}</slot>
                            </li>
                            <li v-else-if="(!options || (options && options.length === 0)) && !loading" class="p-dropdown-empty-message">
                                <slot name="empty">{{emptyMessageText}}</slot>
                            </li>
                            <li v-if="loading"  class="p-dropdown-empty-message">Loading...</li>
							<slot name="loadMore" v-if="!loading"></slot>
