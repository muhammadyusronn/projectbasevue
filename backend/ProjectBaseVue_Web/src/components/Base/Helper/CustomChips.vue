    ...
    methods: {
        ...
        onKeyDown(event) {
            ...
            switch(event.which) {
                ...
                default:
                    if(this.inputSeparator){
                        if (this.separator === ' ' && event.which === 32) {
                            this.addItem(event, inputValue, true);
                        }
                    }
        ...
        onPaste(event) {
            if (this.separator) {
                ...
                if (pastedData) {
                    ...
                    pastedData = pastedData.replaceAll("\n", this.separator).replaceAll("\t", this.separator);
                    let pastedValues = pastedData.split(this.separator);
                    pastedValues = pastedValues.filter(val => (this.allowDuplicate || value.indexOf(val) === -1) && val && val != "");
