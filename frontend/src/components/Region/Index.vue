        ...
        >
          ...
          <template #body="dataModel">
            <Button
              ...
              @click="editor(this.$FORM_MODE_EDIT, dataModel.data.RegionCode)"
            />
            <Button
              icon="pi pi-trash"
              v-show="canDelete"
              class="p-button-rounded p-button-danger mr-2 p-button-sm"
              @click="editor(this.$FORM_MODE_DELETE, dataModel.data.RegionCode)"
            />
          </template>
        </Column>
        <Column field="RegionCode" :header="$t('Region Code')" :sortable="true">
...
export default {
  ...
  data() {
    return {
      ...
      totalRecords: 0,
      start: 0,
      stateKey: this.$STATE_NAME + "region-" + this.$store.getters.getUsername,
      isAdmin: this.$store.getters.getIsAdmin,
      sorts: null,
      lazyParams: {},
      filters: null,
      stateFilters: null,
      editorPath: "/Region/Editor",
      dataUrl: "region/list",
      canCreate: false,
      canEdit: false,
      canDelete: false,
