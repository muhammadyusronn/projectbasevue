<template>
    <IndexBase @auth="onAuth($event)" action="Index" controller="Department"></IndexBase>
        ...
        <template #content>            
            <DataTable ref="dt" responsiveLayout="scroll" class="p-datatable-sm" filterDisplay="row" sortMode="multiple" removableSort
                ...
                <Column field="Created_Date" :header="$t('created_date')" dataType="date" :sortable="true">
                    <template #filter="{filterModel,filterCallback}">
                        <DatePicker :placeholder="$t('created_date')" dateFormat="dd/mm/yyyy" inputMask="99/99/9999" momentFormat="DD/MM/YYYY" class="p-column-filter p-inputtext-sm" v-if="filterModel" v-model="filterModel.value" @keydown.enter="filterCallback()" @date-select="filterCallback()"/>
                    </template>
                    <template #body="{data}">
                ...
                <Column field="Edited_Date" :header="$t('edited_date')" dataType="date" :sortable="true">
                    <template #filter="{filterModel,filterCallback}">
                        <DatePicker dateFormat="dd/mm/yyyy" inputMask="99/99/9999" momentFormat="DD/MM/YYYY" class="p-column-filter p-inputtext-sm" v-if="filterModel" v-model="filterModel.value" @keydown.enter="filterCallback()" placeholder="Filter Edited Date" @date-select="filterCallback()"/>
                    </template>
                    <template #body="{data}">
                ...
                <Column field="Deleted_Date" :header="$t('deleted_date')" :sortable="true">
                    <template #filter="{filterModel,filterCallback}">
                        <DatePicker dateFormat="dd/mm/yyyy" inputMask="99/99/9999" momentFormat="DD/MM/YYYY" class="p-column-filter p-inputtext-sm" v-if="filterModel" v-model="filterModel.value" @keydown.enter="filterCallback()" @date-select="filterCallback()" placeholder="Filter Edited Date" />
                    </template>
                    <template #body="{data}">
    ...
    export default {
        ...
        data() {
            return {
                ...
                totalRecords: 0,
                start: 0,
                stateKey: this.$STATE_NAME +'department-' + this.$store.getters.getUsername,
                isAdmin: this.$store.getters.getIsAdmin,
                sorts: null,
                lazyParams: {},
                filters: null,
                stateFilters: null,
                editorPath: "/Department/Editor",
                dataUrl: "department/list",
                canCreate: false,
                canEdit: false,
                canDelete: false,
                canView: false,
