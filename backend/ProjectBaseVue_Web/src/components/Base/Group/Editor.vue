        ...
        <Card>
            ...
            <template #content>
                <div class="p-inputtext-sm p-fluid">
                    <FormField :labelName="$t('code')" fieldName="Group_Code" v-model="header">
                        <InputText name="Group_Code" :placeholder="$t('code')" v-model="header.Group_Code" type="text"  v-if="header" :disabled="mode == this.$FORM_MODE_VIEW"  />
                    </FormField>

                    <FormField :labelName="$t('description')" fieldName="Group_Description" v-model="header">
                        <InputText name="Group_Description" :placeholder="$t('description')" v-model="header.Group_Description" type="text"  v-if="header" :disabled="mode == this.$FORM_MODE_VIEW"  />
                    </FormField>

                    
                    <div class="col-12">
                        <DataTable :loading="!header" :value="header ? header.Details : null" responsiveLayout="scroll" class="p-datatable-sm editable-cells-table" :rowClass="hideRow">
                            <Column >
                                <template #header="">
                            ...
                            <Column field="Menu_Id" :header="$t('menu')" key="Menu_Id" class="w-12">
                                <template #body="slotProps">
                                    <CustomSelect 
                            ...
                            <Column field="View" :header="$t('view')">
                                <template #body="slotProps">
                                    <Checkbox v-model="slotProps.data.View" :binary="true" :disabled="!canEdit" @change="onCellEditComplete(slotProps)" />
                                </template>
                            </Column>
                             <Column field="Edit" :header="$t('edit')">
                                <template #body="slotProps">
                                    <Checkbox v-model="slotProps.data.Edit" :binary="true" :disabled="!canEdit" @change="onCellEditComplete(slotProps)" />
                                </template>
                            </Column>
                            <Column field="Delete" :header="$t('delete')">
                                <template #body="slotProps">
                                    <Checkbox v-model="slotProps.data.Delete" :binary="true" :disabled="!canEdit" @change="onCellEditComplete(slotProps)" />
                                </template>
                            </Column>
                            <Column field="Create" :header="$t('create')">
                                <template #body="slotProps">
                                    <Checkbox v-model="slotProps.data.Create" :binary="true" :disabled="!canEdit" @change="onCellEditComplete(slotProps)" />
                                </template>
                            </Column>
