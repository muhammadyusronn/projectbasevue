...
namespace ProjectBaseVue_Public_API.Controllers
{
    ...
    public class RegionController : ControllerBase
    {
        ...
        [HttpGet("{RegionCode}/{mode}")]
        public ResultData GetData(string RegionCode, string mode)
        {
            var result = new ResultData();
            ...
            try
            {
                ...
                result = UUtils.CallMiddlewareAPI($"{url}/" + RegionCode.ToString(), userHeaders, "", "GET");
            }
            catch (Exception ex)
            {
                result.success = false;
        ...
        [HttpPost]
        public ResultData SaveData(RegionModel model)
        {
            var result = new ResultData();
