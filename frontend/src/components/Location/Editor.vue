        ...
        <Card>
                        ...
                        <div style="margin-left: 10px" v-if="mode != this.$FORM_MODE_CREATE">
                            ...
                            <OverlayPanel ref="op" appendTo="body" :showCloseIcon="true" id="overlay_panel" style="width: 550px" :breakpoints="{'960px': '80vw'}">
                                ...
                                <div class="grid field" v-if="header.Deleted_Date">
                                    <div class="row">
                                        <i class="pi pi-trash" /> {{this.$formatDateTime(header.Deleted_Date)}}
                                    </div>
                                    <div class="row">
                                        &nbsp;by {{header.Deleted_By}}
                                    </div>
                                </div>
                            </OverlayPanel>
                        </div>
            ...
            <template #content>
                <div class="p-inputtext-sm p-fluid">
                         ...
                         <div class="field grid">
                             <label for="Code" class="col-4">{{$t('code')}}</label>
                                <div class="col-8 md-8">
                                    <InputText name="Code" v-model="header.Code" type="text" :placeholder="$t('code')" v-if="header" 
                                        :class="{'p-invalid': submitted && !header.Code}" 
                                        :disabled="mode != this.$FORM_MODE_CREATE" :readonly="mode != this.$FORM_MODE_CREATE"/>
                            ...
                            <div class="field grid">
                                ...
                                <div class="col-8 md-8">
                                    <InputText name="Name" v-model="header.Description" required="true" type="text" :placeholder="$t('name')" v-if="header" :class="{'p-invalid': submitted && !header.Description}" :disabled="mode != this.$FORM_MODE_CREATE && mode != this.$FORM_MODE_EDIT"/>
                                    <Skeleton v-else-if="!header" />
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="col-12" v-if="this.lengthDet !=0">
                            <DataTable v-if="header" :loading="!header" :value="header.Details ? header.Details : null"
                                ...
                                <Column field="Code_SAP" :header="$t('sap_cod')" Key="sap_cod">
                                    <template #body="slotProps">
                                        <div class="p-fluid">
                                            <InputText name="sap_cod" 
                                                v-model="slotProps.data[slotProps.column.props.field]" required="true"
                                                :placeholder="$t('desc')" @change="onCellEditComplete(slotProps)"
                                                :disabled="true"
                                ...
                                <Column field="Code" :header="$t('code')" Key="Company_Code">
                                    <template #body="slotProps">
                                        <div class="p-fluid">
                                            <InputText name="Code" 
                                                v-model="slotProps.data[slotProps.column.props.field]" required="true"
                                  ...
                                  <Column field="Description" :header="$t('desc')" Key="Description">
                                    <template #body="slotProps">
                                        <div class="p-fluid">
        ...
        methods: {
            getData() {
                ...
                this.$axios.get(this.url + this.id + "/" + this.mode)
                    .then((response) => {
                        self.header = response.data.data.model;
                        self.header.mode = self.mode;
                        self.detail = response.data.data.detail;
                        self.lengthDet = self.detail.length; 
                    })
                    ...
                    .then((response) => {
                        if (response.data.success) {
                            if(this.mode == this.$FORM_MODE_DELETE){
                                 self.$router.push({ name: self.indexName, params: { showToast: true, severity: 'success', summary: 'Data Deleted', detail: "Location " + self.header.Code + " has been deleted successfully", life: 3000 } } );        
                            }else{
                                self.$router.push({ name: self.indexName, params: { showToast: true, severity: 'success', summary: 'Data Saved', detail: "Location " + self.header.Code + " has been saved successfully", life: 3000 } } );  
                            }
