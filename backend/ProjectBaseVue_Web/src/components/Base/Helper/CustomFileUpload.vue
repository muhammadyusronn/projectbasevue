<template>
    <div class="p-fileupload p-fileupload-advanced p-component" v-if="isAdvanced">
        <div class="p-fileupload-buttonbar" v-if="showHeader">
        ...
        <div ref="content" class="p-fileupload-content" @dragenter="onDragEnter" @dragover="onDragOver" @dragleave="onDragLeave" @drop="onDrop">
            ...
            <div class="p-fileupload-files" v-if="hasFiles">
                ...
                <div class="p-fileupload-row" v-for="(file, index) of (viewOnly ? fileModels : files)" :key="file.name + file.type + file.size">
                    <div v-if="isImage(file)" >
                        <img role="presentation" :alt="file.name" :src="file.objectURL" :width="previewWidth" />
                    </div>
                    <div>{{file.name}}</div>
                    <div v-if="viewOnly">
                        <Button type="button" class="p-button-sm p-button-primary" icon="pi pi-download" @click="downloadFile(file)"/>
                    </div>
                    <div v-if="!viewOnly">
...
export default {
    emits: ['select', 'uploader', 'before-upload', 'progress', 'upload', 'error', 'before-send', 'clear', 'remove', 'download'],
    props: {
        name: {
            type: String,
            default: null
    ...
    computed: {
        ...
        hasFiles() {
            return this.viewOnly ? this.fileModels && this.fileModels.length > 0 : this.files && this.files.length > 0;
        },
        chooseDisabled() {
